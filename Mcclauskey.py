...
def EMCclauskey(matriz, variables, filas):  # se ordenan los valores binarios por cantidad de unos presentes
    ...
    iteracion = 0  # se itera la cantidad de variables que hay ininiando desde 0
    while iteracion <= variables:  # se utilizara como control para que se guarden los valores por cantidad de unos
        for a in range(filas):  # se reorre primero por cada fila
        ...
        for a in templist1:  # e comienza a recorrer la primera agrupacion con le fin de sacar una elemento y compararlos con todos los de la agrupacion siguiente
            for b in templist2:  # se recorre la segunda agrupacion para compararla completamente con el elemento de sacado anteiormente
                if ((a.combinacion.count(1) + 1 == b.combinacion.count(1)) or (
                        ...
                        1) + 1)):  # se consulta si estos dos elemetos tienen un bit de diferencia
                    ...
                    ni = nodo(tempbinary, tempdecimal,
                              0)  # se crea el objeto que guarda ambos datos tanto binarios como decimales
...
if __name__ == "__main__":
    ...
    matriz_nueva = minterms(tablaverdad,
                            filas)  # se solicitan los valores decimales donde la funcion es valida y se extraen sus valores binarios corresondientes
    print(
        "--------------------------------------------------------------------------------------------------------------------")
    print("Los minterms son: ", matriz_nueva)
    #dibujar_matriz(matriz_nueva, numero_entradas, filas)
    # listMC=EMCclauskey(matriz_nueva,numero_entradas-1,filas)#primera agrupacion del maclauskey
